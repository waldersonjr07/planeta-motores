...
test('backup da madrugada de hoje está em dia', () => {
  const estado = interpretarMarcador(MARCADOR('2026-08-17T03:00:12-03:00'), AGORA)
...
test('backup de ontem de madrugada ainda está em dia', () => {
  ...
  const estado = interpretarMarcador(MARCADOR('2026-08-16T03:00:12-03:00'), AGORA)
  expect(estado.situacao).toBe('em-dia')
})

test('backup parado há mais de 36 h vira aviso', () => {
  const estado = interpretarMarcador(MARCADOR('2026-08-15T03:00:12-03:00'), AGORA)
  expect(estado).toEqual({
    situacao: 'atrasado',
    feitoEm: new Date('2026-08-15T03:00:12-03:00'),
  })
...
test.each([
  ...
  ['data impossível', '2026-13-45T99:00:12-03:00\n'],
...
test('relógio adiantado não vira aviso', () => {
  ...
  expect(interpretarMarcador(MARCADOR('2026-08-17T14:00:00-03:00'), AGORA).situacao).toBe(
    'em-dia',
  )
})
...
test('o estado da cópia externa não muda o aviso da tela', () => {
  ...
  const pulada = '2026-08-17T03:00:12-03:00  cópia externa: PULADA (rclone não instalado)\n'
